        ...
        <motion.section {...fadeUp} className="relative overflow-hidden py-20 md:py-32 px-6">
          <div className="max-w-5xl mx-auto text-center">
            <h1
              ...
              style={{ fontFamily: `"${t.fontHeading}", sans-serif`, color: t.textColor }}
            >
              {data.title}
            </h1>
            <p className="text-lg md:text-xl opacity-70 max-w-2xl mx-auto mb-10" style={{ color: t.textColor }}>
              {data.subtitle}
            </p>
            <button
              onClick={onCtaClick}
              className="px-8 py-4 text-lg font-semibold text-white transition-all hover:scale-105 active:scale-95"
              ...
              {(data.items || []).map((item: any, i: number) => (
                <div key={i} className="p-6 rounded-xl border border-black/5 bg-white" style={{ borderRadius: t.radius }}>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {item.originalPrice && (
                      <span className="text-sm line-through opacity-40" style={{ color: t.textColor }}>{item.originalPrice?.toLocaleString()} FCFA</span>
                    )}
                  </div>
                </div>
              ))}
          ...
          <div className="max-w-5xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {(data.items || []).map((item: any, i: number) => (
                <div
                  key={i}
                  className="p-8 rounded-xl border-2 text-center flex flex-col"
                  style={{
                    borderColor: item.highlight ? t.primaryColor : "rgba(0,0,0,0.08)",
                    borderRadius: t.radius,
                    backgroundColor: item.highlight ? t.primaryColor + "08" : "white",
                  }}
                >
                  ...
                  {item.originalPrice && (
                    <div className="text-sm line-through opacity-40 mb-4" style={{ color: t.textColor }}>{item.originalPrice?.toLocaleString()} FCFA</div>
                  )}
                  ...
                  <button
                    onClick={onCtaClick}
                    className="w-full py-3 font-semibold text-white transition-all hover:opacity-90"
                    style={{ backgroundColor: t.primaryColor, borderRadius: t.radius }}
                  >
                    Choisir
                  </button>
      ...
      return (
        <motion.section {...fadeUp} className="py-16 md:py-24 px-6">
          <div className="max-w-2xl mx-auto text-center">
            ...
            <h2 className="text-3xl font-bold mb-4" style={{ fontFamily: `"${t.fontHeading}", sans-serif`, color: t.textColor }}>
              {data.title}
            </h2>
    ...
    case "cta":
      return (
        <motion.section {...fadeUp} className="py-20 md:py-28 px-6 text-center" style={{ backgroundColor: t.primaryColor + "08" }}>
          <div className="max-w-3xl mx-auto">
            <h2 className="text-3xl md:text-4xl font-bold mb-4" style={{ fontFamily: `"${t.fontHeading}", sans-serif`, color: t.textColor }}>
              {data.title}
            </h2>
            <p className="text-lg opacity-70 mb-8" style={{ color: t.textColor }}>{data.subtitle}</p>
            <button
              onClick={onCtaClick}
              className="px-10 py-4 text-lg font-semibold text-white transition-all hover:scale-105 active:scale-95 shadow-lg"
              style={{ backgroundColor: t.primaryColor, borderRadius: t.radius }}
            >
              {data.ctaText}
            </button>
      ...
      return (
        <motion.section {...fadeUp} className="py-16 md:py-24 px-6">
          <div className="max-w-md mx-auto text-center">
            <h2 className="text-2xl md:text-3xl font-bold mb-6" style={{ fontFamily: `"${t.fontHeading}", sans-serif`, color: t.textColor }}>
              {data.title}
            </h2>
            {data.incentive && (
              <p className="text-sm mb-6 font-medium" style={{ color: t.primaryColor }}>{data.incentive}</p>
            )}
            {data.spotsText && (
              <p className="text-sm mb-4 font-medium" style={{ color: t.primaryColor }}>{data.spotsText}</p>
            )}
            <div className="flex gap-2">
              <input
                type="text"
                placeholder={data.placeholder}
                className="flex-1 px-4 py-3 border border-black/10 text-sm bg-white"
                style={{ borderRadius: t.radius, color: t.textColor }}
              />
              <button
                className="px-6 py-3 font-semibold text-white text-sm whitespace-nowrap"
                style={{ backgroundColor: t.primaryColor, borderRadius: t.radius }}
              >
                {data.buttonText}
              </button>
        ...
        <motion.section {...fadeUp} className="py-16 md:py-24 px-6">
          <div className="max-w-5xl mx-auto">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {/* Products will be injected dynamically from the collection */}
              <div className="aspect-square rounded-xl bg-black/5 flex items-center justify-center text-sm opacity-40" style={{ borderRadius: t.radius }}>
...
function CountdownSection({ data, theme: t }: { data: any; theme: Props["theme"] }) {
  ...
  return (
    <motion.section {...fadeUp} className="py-16 px-6 text-center" style={{ backgroundColor: t.primaryColor + "08" }}>
      <h2 className="text-2xl md:text-3xl font-bold mb-8" style={{ fontFamily: `"${t.fontHeading}", sans-serif`, color: t.textColor }}>
        {data.title}
      </h2>
...
function FaqItem({ q, a, theme: t }: { q: string; a: string; theme: Props["theme"] }) {
  ...
  return (
    <div className="border border-black/5 rounded-xl overflow-hidden bg-white" style={{ borderRadius: t.radius }}>
      ...
      {open && (
        <div className="px-5 pb-5 text-sm opacity-60" style={{ color: t.textColor }}>{a}</div>
      )}
    </div>
  );
}
