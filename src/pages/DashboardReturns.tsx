...
export default function DashboardReturns() {
  ...
  const fetchReturns = async () => {
    ...
    const { data } = await supabase
      ...
      .select("id, reason, description, status, created_at, order_id, buyer_id, orders(order_number)")
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-foreground tracking-tight">Retours</h1>
        <p className="text-sm text-muted-foreground mt-1">{returns.length} demande{returns.length > 1 ? "s" : ""} de retour</p>
      </div>
              ...
              <div className="flex items-start justify-between">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-sm font-medium text-foreground">{reasonLabels[r.reason] || r.reason}</span>
                    <Badge className={`text-[10px] ${statusColors[r.status]}`}>{statusLabels[r.status]}</Badge>
                  </div>
                  <div className="flex items-center gap-2 text-[10px] text-muted-foreground">
                    {r.orders && <span>Commande #{(r.orders as any).order_number}</span>}
